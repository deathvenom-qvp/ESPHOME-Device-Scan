...
(function () {
  ...
  /* Builds the confirmation body from the plan, so the warning states what
   * will really happen rather than a generic "are you sure". Resolves with
   * "all", "skip" or null (cancelled). */
  function confirmRegenerateAll(plan) {
    el.confirmBody.textContent = "";

    var lead = node("div", null,
      "This rebuilds " + plan.total + " device config"
      + (plan.total === 1 ? "" : "s") + " from their parent templates.");
    el.confirmBody.appendChild(lead);

    var list = node("ul");
    if (plan.untouched.length) {
      list.appendChild(node("li", null,
        plan.untouched.length + " generated by this add-on — safe to rebuild"));
    }
    if (plan.missing.length) {
      list.appendChild(node("li", null,
        plan.missing.length + " with no config yet — will be created"));
    }
    if (plan.unmatched.length) {
      list.appendChild(node("li", null,
        plan.unmatched.length + " with no parent template — skipped"));
    }
    if (list.childNodes.length) el.confirmBody.appendChild(list);

    if (plan.edited.length) {
      var warn = node("div", "warn-block");
      warn.appendChild(node("div", null,
        "⚠ " + plan.edited.length + " config"
        + (plan.edited.length === 1 ? " was" : "s were")
        + " written or edited by hand. Rebuilding replaces those changes:"));
      warn.appendChild(node("div", "names",
        plan.edited.slice(0, 12).join(", ")
        + (plan.edited.length > 12 ? ", …" : "")));
      el.confirmBody.appendChild(warn);
    }

    el.confirmBody.appendChild(node("div", "note",
      "Every replaced file is backed up alongside it as .bak-<timestamp>. "
      + "Parent templates are never touched."));

    el.confirmSkip.hidden = plan.edited.length === 0;
    el.confirm.hidden = false;

    return new Promise(function (resolve) {
      function finish(result) {
        el.confirm.hidden = true;
        el.confirmGo.onclick = el.confirmSkip.onclick = el.confirmCancel.onclick = null;
        document.removeEventListener("keydown", onKey);
        resolve(result);
      }
      function onKey(event) { if (event.key === "Escape") finish(null); }

      el.confirmGo.onclick = function () { finish("all"); };
      el.confirmSkip.onclick = function () { finish("skip"); };
      el.confirmCancel.onclick = function () { finish(null); };
  ...
  function regenerateAll() {
    el.regenAllBtn.disabled = true;
    el.regenAllBtn.textContent = "Checking…";

    api("api/regenerate-all/plan")
      .then(function (plan) {
        // The plan has landed; the button stays disabled while the dialog is
        // up, but "Checking…" is no longer true.
        el.regenAllBtn.textContent = "Regenerate all";
        if (!plan.total) {
          banner("Nothing to regenerate: no device has a matching parent template.", "warn");
          return null;
        }
        return confirmRegenerateAll(plan);
      })
      .then(function (choice) {
        if (!choice) return null;
        el.regenAllBtn.textContent = "Regenerating…";
        var query = choice === "skip" ? "?skip_edited=1" : "";
        return api("api/regenerate-all" + query, { method: "POST" })
          .then(function (data) { applyScan(data); return refresh(); });
      })
      .catch(function (err) { banner("Regenerate all failed: " + err.message); })
      .then(function () {
        el.regenAllBtn.disabled = false;
        el.regenAllBtn.textContent = "Regenerate all";
        pollLogs();
      });
  }
